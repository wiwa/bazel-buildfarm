 ...
 * <p>
 * Currently has special code for discriminating between Javac/Scalac, and other persistent workers.
 */
public class PersistentExecutor {
  ...
  private static final ProtoCoordinator coordinator = ProtoCoordinator.ofCommonsPool(
      getMaxWorkersPerKey());
  ...
  private static ImmutableList<String> parseInitCmd(String cmdStr, ImmutableList<String> argsList) {
    if (!cmdStr.endsWith(PERSISTENT_WORKER_FLAG)) {
      throw new IllegalArgumentException(
          "Persistent Worker request must contain " + PERSISTENT_WORKER_FLAG +
              "\nGot: parseInitCmd[" + cmdStr + "]" +
              "\n" + argsList
      );
    }

    String cmd = cmdStr.strip().substring(0,
        (cmdStr.length() - PERSISTENT_WORKER_FLAG.length()) - 1);
